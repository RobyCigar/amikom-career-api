...
export default async (req: NextApiRequest, res: NextApiResponse) => {
	...
	try {
		...
		// PROD 
		const getData: any = await axios.get<AxiosResponse>(url).catch(function (error) {
			if (error.response) {
		...
		// get image url
		const imgUrl: any = $('div.thumb>img').map((_i, x) => {
			return $(x).attr('src');
		}).toArray();

		// get the title of jobs
		title = $("h2").text().split("\n")
		title = title.filter((_el, i) => {
		...
		// get slug
		slug = $(`a[class="detail-link"]`).map((_i, x) => $(x).attr('href')).toArray()
